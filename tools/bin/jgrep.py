...
def parse_args():
   parser = OptionParser(usage="%prog <options>", version="0.1", description="grep recursivo para archivos zip")

   parser.add_option("-g", "--grep", dest="grep", help="Patron de busqueda en el contenido de los archivos.")
   parser.add_option("-f", "--file-pattern", dest="filepattern", help="Patron de filtrado de archivos dentro del zip.")
   ...
   if len(args) != 1:
      ...
      options["filepattern"] = re.compile(opts.filepattern) if opts.filepattern is not None else None
...
def search(parent, file, options):
   ...
   for child in file.namelist():
      ...
      try:
         matches.extend(search(os.path.join(parent, child), ZipFile(child_file), options))
      except BadZipfile:
